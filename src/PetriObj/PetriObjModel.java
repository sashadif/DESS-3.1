...
public class PetriObjModel implements Serializable {
     ...
     */
    public void go(double timeModeling, JTextArea area,PetriNetsPanel panel) { //виведення протоколу подій та результатів моделювання у об"єкт класу JTextArea
        ...
        while (t < timeModeling) {
          ...
          PetriSim.setTimeCurr(t); // просування часу //3.12.2015
            ...
            if (t <= timeModeling) {
                ...
                if (isProtokolPrint == true) {
                    area.append(" Обраний об'єкт  " + conflictObj.get(num).getName() + "\n" + " NextEvent " + "\n");
                }

                for (int i = 0; i < listObj.size(); i++) {
                    if (listObj.get(i).getNumObj() == conflictObj.get(num).getNumObj()) {
                        if (isProtokolPrint == true) {
                            area.append(" time =   " + t + "   Подія '" + listObj.get(i).getEventMin().getName() + "'\n"
                                    + "                       відбувається для об'єкту   " + listObj.get(i).getName() + "\n");
                        }
                        listObj.get(i).doT();
                        listObj.get(i).stepEvent(panel);
                ...
                for (PetriSim e : listObj) {
                    //можливо змінились умови для інших обєктів
                    e.input(panel); //вхід маркерів в переходи Петрі-об'єкта
